...
tickers = ['MMM', 'ABT', 'ABBV', 'ABMD', 'ACN', 'ATVI', 'ADBE', 'AMD', 'AAP', 'AES', 'AFL', 'A', 'APD', 'AKAM', 'ALK', 'ALB', 'ARE', 'ALXN', 'ALGN', 'ALLE']


data = yf.download(tickers, start="2020-01-01", end="2024-05-27", group_by="ticker")
dataNewer = yf.download(tickers, start="2022-01-01", end="2024-05-27", group_by="ticker")
dataOlder = yf.download(tickers, start="2000-01-01", end="2024-05-27", group_by="ticker") 
...
for ticker in tickers:
        try:
                ...
                model = SARIMAX(df['Close'], exog=df['Volume'], order=(1,2,1), seasonal_order=(1,1,1,12))
                result = model.fit()
                forecast_steps = 180
...
for ticker in tickers:
        try:
                prices_dict_Older[ticker] = {}
                df = dataOlder[ticker][['Close', 'Volume']]
                prices_dict_Older[ticker]["Now"] = df['Close'][-1]
                model = SARIMAX(df['Close'], exog=df['Volume'], order=(1,1,2), seasonal_order=(0,0,0,12))
                result = model.fit()
                forecast_steps = 180
                forecast = result.get_forecast(steps=forecast_steps, exog=df['Volume'][-forecast_steps:])
                forecast_prices = forecast.predicted_mean
                forecast_dates = pd.date_range(start=df.index[-1], periods=forecast_steps + 1, freq='D')
                prices_dict_Older[ticker]["Prediction"] = forecast_prices[-1]
        except:
                pass
...
for ticker in tickers:
        try:
                prices_dict[ticker] = {}
                df = dataNewer[ticker][['Close', 'Volume']]
                prices_dict[ticker]["Now"] = df['Close'][-1]
                model = SARIMAX(df['Close'], exog=df['Volume'], order=(1,1,2), seasonal_order=(0,0,0,12))
                result = model.fit()
                forecast_steps = 180
                ...
                forecast_prices = forecast.predicted_mean
                forecast_dates = pd.date_range(start=df.index[-1], periods=forecast_steps + 1, freq='D')
                prices_dict[ticker]["Prediction"] = forecast_prices[-1]
        except:
                pass
...
investments = {
    'Seagate Technology Holdings plc (STX)': 25,
    'Monolithic Power Systems, Inc. (MPWR)': 15,
    'Jacobs Engineering Group Inc. (J)': 10,
    '(GEN)': 10,
    'The Estée Lauder Companies Inc. (EL)': 10,
    'Enphase Energy, Inc. (ENPH)': 10,
    '(MSCI)': 5,
    'CVS Health Corporation (CVS)': 5,
